...
function isCodexProviderName(
  ...
  return providerRegistry.getOrNull(provider)?.group === "codex";
}
...
function extractContextUsageFromSDKMessages(
  ...
  const isCodexProvider = isCodexProviderName(provider);
...
export function createSessionsRoutes(deps: SessionsDeps): Hono {
  ...
  routes.get("/projects/:projectId/sessions/:sessionId", async (c) => {
    ...
    if (
      !loadedSession &&
      providerRegistry.getOrNull(project.provider)?.group === "claude" &&
    ...
    if (
      !loadedSession &&
      (providerRegistry.getOrNull(project.provider)?.group === "claude" ||
        providerRegistry.getOrNull(project.provider)?.group === "codex") &&
    ...
    try {
      ...
      if (shouldCloneFromCodex) {
        ...
        cloneProvider =
          ...
          body.provider ??
          (isCodexProviderName(project.provider)
            ? (project.provider as ProviderName)
            : "codex");
